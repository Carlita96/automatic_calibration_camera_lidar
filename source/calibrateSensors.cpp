...
void CalibrateSensors::initParameters(ros::NodeHandle &nodeHangle) 
{
    ...
    */
    ...
    nodeHandle.param<bool>( "calibration/analysisData/saveObservations", mAnalysisDataSaveObservations, false );
    // Spherical target information
    nodeHandle.param<double>( "calibration/sphericalTarget/radius", mSphericalTargetRadius, 0.5 );
    // Output files directory
    nodeHandle.param<std::string>( "output_files_irectory", mOutputAnalysisFilesDirectory, std::string( "./" ));
    nodeHandle.param<std::string>( "calibration_params_directory", mOutputCalibrationParamsDirectory, std::string( "./" ));
...
void CalibrateSensors::dataCallback(const sensor_msgs::ImageConstPtr& pImageMsg, const sensor_msgs::PointCloud2::ConstPtr& pPointcloudMsg)
{
    ...
    */
    ...
    if (pointCenterSpherePointCloud != Eigen::Vector3d(0,0,0) && pointCenterSphereImage != Eigen::Vector3d(0,0,0)
        ...
        ROS_INFO_STREAM("FINDING SPHERICAL TARGET: There are: " << mListPointsCenterSphericalTargetPointCloud.size() 
                << " spherical positions in list. Missing: " << mNumberPositions-mListPointsCenterSphericalTargetPointCloud.size() << ".");
...
int CalibrateSensors::calibrateSensors()
{
    ...
    */
    ...
    Eigen::Vector3d pointCloudPoint, imagePoint;
    Eigen::Vector2d imagePoint2d;
    int i;
    if (mAnalysisDataSaveObservations) {
        for (auto it = std::begin(listPointCloudWoOutliers); it != std::end(listPointCloudWoOutliers); ++it) { 
            // Get point from list
            i = it - listPointCloudWoOutliers.begin();
            pointCloudPoint = listPointCloudWoOutliers[i];
            imagePoint = listImageWoOutliers[i];
    
            std::string outputFile = mOutputAnalysisFilesDirectory;
            outputFile += "observations3D3D_" + mStartingDatetime + ".txt";
            mOutfile.open(outputFile, std::ios_base::app);
            mOutfile << pointCloudPoint[0] << " " << pointCloudPoint[1] << " " << pointCloudPoint[2] << " "
                << imagePoint[0] << " " << imagePoint[1] << " " << imagePoint[2] <<"\n" ; 
            mOutfile.close();
        }
        for (auto it = std::begin(listPointCloudWoOutliers2); it != std::end(listPointCloudWoOutliers2); ++it) { 
            // Get point from list
            i = it - listPointCloudWoOutliers2.begin();
            pointCloudPoint = listPointCloudWoOutliers2[i];
            imagePoint2d = listImagePixelWoOutliers[i];
    
            std::string outputFile = mOutputAnalysisFilesDirectory;
            outputFile += "observations3D2D_" + mStartingDatetime + ".txt";
            mOutfile.open(outputFile, std::ios_base::app);
            mOutfile << pointCloudPoint[0] << " " << pointCloudPoint[1] << " " << pointCloudPoint[2] << " "
                << imagePoint2d[0] << " " << imagePoint2d[1] <<"\n" ; 
            mOutfile.close();
        }
    }
    ...
    if (status == 0) {
        ...
        // If testing publish the TF from frames to each other
        if (status == 0) {
            // Get transform
            geometry_msgs::Transform staticTransform;
            staticTransform.translation.x = translationVector[0];
            staticTransform.translation.y = translationVector[1];
            staticTransform.translation.z = translationVector[2];
            staticTransform.rotation.x = quaternions.x();
            staticTransform.rotation.y = quaternions.y();
            staticTransform.rotation.z = quaternions.z();
            staticTransform.rotation.w = quaternions.w();

            // Get inverse
            tf::Transform transform;
            tf::transformMsgToTF(staticTransform, transform);
            geometry_msgs::Transform invertedTransform;
            tf::transformTFToMsg(transform.inverse(), invertedTransform);

            ROS_INFO_STREAM("Transform Camera -> LiDAR: \n\t- Tx: " << invertedTransform.translation.x
                << "\n\t- Ty: " << invertedTransform.translation.y << "\n\t- Tz: " << invertedTransform.translation.z
                << "\n\t- Qx: " << invertedTransform.rotation.x << "\n\t- Qy: " << invertedTransform.rotation.y
                << "\n\t- Qz: " << invertedTransform.rotation.z << "\n\t- Qw: " << invertedTransform.rotation.w);

            // Save parameters
            Eigen::Vector3d translationVector2 = Eigen::Vector3d(invertedTransform.translation.x, invertedTransform.translation.y,
                invertedTransform.translation.z);
            Eigen::Quaterniond quaternions2 = Eigen::Quaterniond(invertedTransform.rotation.x, invertedTransform.rotation.y, 
                invertedTransform.rotation.z, invertedTransform.rotation.w);
            ...
            if (mTesting) {
                ...
                staticTransformStamped.header.frame_id = "cam_rear";
                staticTransformStamped.child_frame_id = "velodyne";
                staticTransformStamped.transform.translation.x = translationVector[0];
                staticTransformStamped.transform.translation.y = translationVector[1];
                staticTransformStamped.transform.translation.z = translationVector[2];
                staticTransformStamped.transform.rotation.x = quaternions.x();
                staticTransformStamped.transform.rotation.y = quaternions.y();
                staticTransformStamped.transform.rotation.z = quaternions.z();
                staticTransformStamped.transform.rotation.w = quaternions.w();
                mStaticBroadcaster.sendTransform(staticTransformStamped);
            } else {
                // Otherwise, save calibration parameters to file
                std::string outputFile = mOutputCalibrationParamsDirectory;
                outputFile += "calibrationParameters.yaml";
                mOutfile.open(outputFile, std::ios_base::out);
                mOutfile << "Tx : " << translationVector[0] << "\n";
                mOutfile << "Ty : " << translationVector[1] << "\n";
                mOutfile << "Tz : " << translationVector[2] << "\n";
                mOutfile << "Qx : " << quaternions.x() << "\n";
                mOutfile << "Qy : " << quaternions.y() << "\n";
                mOutfile << "Qz : " << quaternions.z() << "\n";
                mOutfile << "Qw : " << quaternions.w() << "\n";
                mOutfile.close();
            }
        }
    }
